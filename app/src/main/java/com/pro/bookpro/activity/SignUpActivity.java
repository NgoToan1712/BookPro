...
import com.pro.bookpro.databinding.ActivitySignUpBinding;
...
public class SignUpActivity extends BaseActivity {
        ...
        firebaseAuth.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(this, task -> {
                    ...
                    if (task.isSuccessful()) {
                        ...
                        if (user != null) {
                            User userObject = new User(user.getUid(),
                                    mActivitySignUpBinding.edtName.getText().toString(),
                                    mActivitySignUpBinding.edtAddress.getText().toString(),
                                    mActivitySignUpBinding.edtPhone.getText().toString(),
                                    user.getEmail(),
                                   // password,
                                    "***",
                                    false);
                            createDataFirebaseUser(userObject);
    ...
    private void createDataFirebaseUser(User user){
        FirebaseDatabase.getInstance(Constant.FIREBASE_URL).getReference().child("User").child(user.getId()).setValue(user);
    }
}